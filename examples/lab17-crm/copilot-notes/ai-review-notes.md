...
Here's one more test for CustomerServiceTest:
@Test
...
LocalDateTime.now()
    ...
    assertThrows(UnsupportedOperationException.class, () -> list.add(
        new Customer("CUS-1002", "Ravi Singh", "ravi@northstar.com", "+1-555-0102", CustomerStatus.PROSPECT, LocalDateTime.now())
    ));
}
This test verifies that listAll() returns a truly unmodifiable list — callers can't accidentally modify the internal state by adding/removing customers
